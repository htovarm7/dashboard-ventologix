...
webApi = APIRouter(prefix="/web", tags=["web"])
...
def get_raw_data_excel(id_cliente: int = Query(..., description="ID del cliente")):
    try:
        ...
        return StreamingResponse(
            output,
            media_type="webApilication/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
...
"""
# Check this endpoint
@webApi.get("/line-data-proc-date")
...
def get_compressor_data(id_cliente: int = Query(..., description="ID del cliente")):
    try:
        ...
        cursor.execute(f"SELECT hp, tipo, voltaje, marca, numero_serie, Alias FROM compresores WHERE id_cliente = {id_cliente} and linea= 'A'")
        results = cursor.fetchall()
