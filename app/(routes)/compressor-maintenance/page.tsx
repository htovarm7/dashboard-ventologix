...
const CompressorMaintenance = () => {
  ...
  const handleAddMaintenance = async (
    ...
    try {
      // Encontrar el compresor seleccionado para obtener su tipo
      const selectedCompressor = allCompresores.find(
        (comp) => comp.id === maintenanceData.compressorId
      );

      if (!selectedCompressor) {
        throw new Error("Compresor no encontrado");
      }

      // Obtener tipos de mantenimiento para determinar el tipo correcto
      ...
      const typesData: MaintenanceTypesResponse = await typesResponse.json();

      // Encontrar el tipo correspondiente
      const maintenanceType = typesData.maintenance_types.find(
        (type) => type.nombre_tipo === maintenanceData.type
      );
      ...
      showSuccess(
        ...
        "El mantenimiento se ha registrado exitosamente en la base de datos."
      );
    } catch (error) {
      console.error("Error al agregar mantenimiento:", error);
      showSuccess(
        "Error",
        "Hubo un error al agregar el mantenimiento. Por favor intenta de nuevo."
      );
    }
  };
