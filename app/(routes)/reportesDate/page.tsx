  ...
  const fetchData = async (id: string, linea: string, date: string) => {
    try {
      const [pieRes, lineRes, commentsRes, statsRes, clientRes, compressorRes] = await Promise.all([
        (async () => {
          const res = await fetch(`http://127.0.0.1:8000/report/pie-data-proc-day?id_cliente=${id}&linea=${linea}&date=${date}]`);
          return res.json();
        })(),
        (async () => {
          const res = await fetch(`http://127.0.0.1:8000/report/line-data-proc-day?id_cliente=${id}&linea=${linea}&date=${date}]`);
          return res.json();
        })(),
        (async () => {
          const res = await fetch(`http://127.0.0.1:8000/report/comments-data-day?id_cliente=${id}&linea=${linea}&date=${date}]`);
          return res.json();
        })(),
        (async () => {
          const res = await fetch(`http://127.0.0.1:8000/report/stats-data-day?id_cliente=${id}&linea=${linea}&date=${date}]`);
          return res.json();
        })(),
        (async () => {
        ...
        {Off == 100 ? (
          ...
          <div className="gap-10 items-left justify-left text-left">
            ...
            <p className="text-xl text-left">
              • El día <strong>{shortDate}</strong> se iniciaron labores a las <strong>{firstHour}</strong> y se concluyeron a las <strong>{lastHour}</strong>
            </p>
